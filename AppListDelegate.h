...
class NoHoverDelegate: public QStyledItemDelegate {
	...
	static void showPaletteTable( QWidget* parent, const QWidget* w ) {

		auto* dlg = new QDialog( parent );
		dlg->setWindowTitle( "Palette" );

		auto* t = new QTableWidget( dlg );
		t->setColumnCount( 4 );
		t->setHorizontalHeaderLabels( { "Role", "Active", "Inactive", "Disabled" } );
		t->horizontalHeader()->setStretchLastSection( true );
		t->verticalHeader()->setVisible( false );

		struct R {
			QPalette::ColorRole r; const char* n;
		} roles[] = {
	...
	{ QPalette::Highlight, "Highlight" }, { QPalette::HighlightedText, "HighlightedText" },
	{ QPalette::Link, "Link" }, { QPalette::LinkVisited, "LinkVisited" },
	...
	{ QPalette::Dark, "Dark" }, { QPalette::Mid, "Mid" }, { QPalette::Shadow, "Shadow" },
		...
		t->setRowCount( int( std::size( roles ) ) );

		auto setCell = [&] ( int row, int col, const QColor& c ) {

			auto* it = new QTableWidgetItem();
			it->setText( c.name( QColor::HexArgb ).toUpper() );
			it->setBackground( QBrush( c ) );
			it->setFlags( it->flags() & ~( Qt::ItemIsEditable | Qt::ItemIsSelectable ) );
			t->setItem( row, col, it );
			};

		for( int r = 0; r < int( std::size( roles ) ); ++r ) {

			t->setItem( r, 0, new QTableWidgetItem( roles[ r ].n ) );
			setCell( r, 1, pal.color( QPalette::Active, roles[ r ].r ) );
			setCell( r, 2, pal.color( QPalette::Inactive, roles[ r ].r ) );
			setCell( r, 3, pal.color( QPalette::Disabled, roles[ r ].r ) );
		}
